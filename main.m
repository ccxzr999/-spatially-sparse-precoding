...
tryNumber = 100; 
%% Parameters
parameters_main = containers.Map('KeyType','char','ValueType','any');%创建了一个新的containers.Map实例，并指定了键类型（KeyType）为字符（char），值类型（ValueType）为任意类型（any）。
parameters_main("numberTransmitAntennas") = 64; % Number of transmit antennas parameters_main['key1'] = 'value1';  % 使用[]操作符插入键值对
parameters_main("numberRecieveAntennas") = 16; % Number of receive antennas
parameters_main("numberDataStreams") = 1; % Number of data streams
parameters_main("numberRFchains") = 4; % Number of RF chains for precoding and combining
parameters_main("numberCluster") = 8; % Number of clusters
parameters_main("numberRayPercluster") = 10; % Number of rays per cluster
parameters_main("angularSpread") = 7.5; % Angular spread of 7.5 degree
...
for s = 1 :length(snrValues)
    SNR = snrValues(s);
    for i = 1:tryNumber
        channel = channel_generation(parameters_main);
