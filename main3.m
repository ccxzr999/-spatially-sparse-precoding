...
%% Parameters
parameters_main3 = containers.Map('KeyType','char','ValueType','any');%创建了一个新的containers.Map实例，并指定了键类型（KeyType）为字符（char），值类型（ValueType）为任意类型（any）。
parameters_main3("numberTransmitAntennas") = 64; % Number of transmit antennas parameters_main['key1'] = 'value1';  % 使用[]操作符插入键值对
